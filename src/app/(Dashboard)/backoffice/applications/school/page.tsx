...
export default function SchoolApplications() {
  ...
  useEffect(() => {
    setSearchQuery(`${searchData.fullName && `fullName=${searchData.fullName}`}${(searchData.from && searchData.to) && `&from=${searchData.from}`}${(searchData.from && searchData.to) && `&to=${searchData.to}`}${(searchData.status) && `&status=${searchData.status}`}`);
  }, [ searchData ]);
...
console.log(searchQuery)
  ...
  const initiateDelete = (item: any): void => {
    setItemToDelete({ id: item?.schoolApplicationId, name: `${item?.personalInformation?.firstName} ${item?.personalInformation?.lastName}` });
  ...
  const getNextPage = (): void => {
    setSearchQuery("");
    getApplicationsTrigger({ direction: "forward", cursor: data?.nextCursor})
  }
  ...
  return (
    <main className="p-3 md:p-3 lg:p-4 xl:p-6">
      <section className="flex flex-row gap-3 max-md:flex-wrap">
        ...
        <div className="left basis-1/2 bg-transparent">
          <Link href="/backoffice/applications/school/add" className="bg-[#FF4512] max-w-max max-md:mr-auto md:ml-auto flex flex-row font-medium mt-1 items-center gap-1.5 rounded-lg px-6 py-2.5 shadow-md text-white">
      ...
      <section className="mt-5">
                ...
                // (isLoading || (searchQuery && isFetching))
                  ...
                  ? (
                    ...
                    data?.data && data?.data.map((each: any, index: number) => (
                      <tr key={index} className="text-neutral-500 border-b center border-b-zinc-200/70 w-full grid md:table-row grid-cols-3 font-light">
                        {/* <td>{String(new Date(each?.createdAt)).split(" ").slice(0, 4).join(" ")}</td> */}
                        <td>{formatDate(each?.createdAt?.split("T")[0])}</td>
                        <td className="flex flex-col">
                          <span className="text-zinc-600 tracking-wide font-medium">{each?.personalInformation?.firstName} {each?.personalInformation?.lastName}</span>
                          <span className="text-zinc-500 text-sm">{each?.contactInformation?.email}</span>
                        </td>
                        ...
                        <td className="flex flex-row gap-1.5 lg:gap-2 xl:gap-2.5" style={{ width: "110px"}}>
                          ...
                          <Link href={`/backoffice/applications/school/add?id=${each.schoolApplicationId}&action=update`} className="">
                            <svg xmlns="http://www.w3.org/2000/svg" width="21" height="20" viewBox="0 0 21 20" fill="none">
        ...
        <div className="flex items-center max-md:max-w-[calc(100vw-2.2rem)] w-[500px] bottom-4 bg-white mb-4 ml-auto  md:max-w-max rounded-md  mt-8 p-1">
          <button onClick={getPreviousPage} disabled={data?.previousCursor ? false : true} className={`${data?.previousCursor ? "text-stone-700" : "text-stone-300"} border md:mr-5 border-slate-400/80 flex gap-x-1 items-center bg-primary-pale/30 rounded-md text-primary/90 pl-3 pr-4 max-md:pl-2 max-md:pr-3 py-1`}>
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4">
              <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 19.5 8.25 12l7.5-7.5" />
            </svg>
            Previous
          </button>
          <div className=" text-ellipsis text-slate-600  max-w-none sm:max-w-[180px] md:max-w-none whitespace-nowrap overflow-hidden">
            {
          ...
          {/* <div className="flex flex-row -ml-0.5 text-slate-500">
            <button  className="bg-primary-pale/30 border border-white hover:border-primary-red rounded-md duration-300 text-primary/90 px-3 py-1">{paginationData?.data?.numPages}</button>

            {
              Array.from({ length: 2}).map((_, index: number) => (
                <button key={index} className="bg-primary-pale/30 border border-white hover:border-primary-red rounded-md duration-300 text-primary/90 px-3 py-1">1{index+1}</button>
              ))
            }
          </div> */}
          <button onClick={getNextPage} disabled={data?.nextCursor ? false : true} className={`${data?.nextCursor ? "text-stone-700" : "text-stone-300"} border md:ml-5 border-slate-400/80 flex gap-x-1 items-center bg-primary-pale/30 rounded-md text-primary/90 pl-4 pr-3 max-md:pl-3 max-md:pr-2 py-1`}>
            Next
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4">
          ...
          {/* <button onClick={getPreviousPage} className="bg-primary-pale/30 flex flex-row gap-1.5 hover:bg-primary-pale rounded-tl-md items-center rounded-bl-md text-stone-600 px-3 py-0.5">
          ...
          <button onClick={getNextPage} className="bg-primary-pale/30 flex flex-row items-center gap-1.5 hover:bg-primary-pale rounded-tr-md rounded-br-md text-stone-600 px-3 py-0.5">
            Next
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-4 h-4">
