...
export default function Schools() {
  ...
  const [ searchData, setSearchData ] = useState({ location: "", program: "", from: "", to: "", name: "", cursor: "" });
  const [ isDeleteModalOpen, setIsDeleteModalOpen ] = useState<boolean>(false);
  const [ itemToDelete, setItemToDelete ] = useState({ id: "", name: "" });

  const [ getSchoolsTrigger, { isLoading, isFetching, data, error }] = searchQuery ? api.adminApis.useLazyFilterSchoolsQuery() : api.adminApis.useLazyGetSchoolsQuery();
  const [ deleteTrigger, { isLoading: isDeleting, error: deleteError, isSuccess: isDeleteSuccess, reset } ] = api.adminApis.useDeleteSchoolMutation()
  ...
  useEffect(() => {
    setSearchQuery(`${searchData.location && `location=${searchData.location}`}${searchData.program && `&program=${searchData.program}`}${searchData.name && `&name=${searchData.name}`}${(searchData.from && searchData.to) && `&from=${searchData.from}`}${(searchData.to && searchData.from) && `&to=${searchData.to}`}${(searchData.cursor) && `&cursor=${searchData.cursor}`}`);
  }, [ searchData ]);

  const handleFilterTextInput = (e: ChangeEvent<HTMLInputElement|HTMLSelectElement|any>|any): void => {
    e.preventDefault();
                  ...
                  ? (
                    ...
                    data?.data && data?.data.map((each: any, index: number) => (
                      <tr key={index} className="text-slate-500 border-b border-b-zinc-200/70 w-full grid md:table-row grid-cols-3 font-semi-bold">
                        ...
                        <td>{getCountryNameFromCode(each?.info?.country)}</td>
                        <td className="capitalize">{each?.program?.programType?.replace(/_/g, " ")?.toLowerCase()}</td>
                        <td className="flex flex-row gap-1.5 lg:gap-2 xl:gap-2.5" style={{ width: "110px"}}>
                          <Link href={`/backoffice/schools/add?id=${each.schoolId}&action=view`} className="">
                          ...
                          <Link href={`/backoffice/schools/add?id=${each.schoolId}&action=update`} className="">
                            <svg xmlns="http://www.w3.org/2000/svg" width="21" height="20" viewBox="0 0 21 20" fill="none">
